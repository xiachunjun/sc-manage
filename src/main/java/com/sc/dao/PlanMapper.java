...
public interface PlanMapper {
	...
	@Select({
		"select id, ref_dept, ref_user, ref_position, plan_name, plan_type, plan_date, plan_main_person, ",
			"plan_content, plan_begin_time, plan_end_time,rate_of_progress, progress_info, ",
			"check_user, check_time ",
		"from sc_plans ",	
		"where plan_type=#{planType,jdbcType=INTEGER}"	
	})
	@Results({
        @Result(column="id", property="id", jdbcType=JdbcType.INTEGER, id=true),
        @Result(column="ref_dept", property="refDept", jdbcType=JdbcType.VARCHAR),
        @Result(column="ref_user", property="refUser", jdbcType=JdbcType.INTEGER),
	...
	List<Plan> selectListByPlanType(Integer planType);
