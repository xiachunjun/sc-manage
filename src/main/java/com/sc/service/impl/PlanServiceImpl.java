...
public class PlanServiceImpl implements IPlanService {
	...
	public void addPlan(PlanModel planModel) {
		...
		int flag = planMapper.insertSelective(record);
		if (flag <= 0) {
			throw new ScException("新增出错");
		}
	}
...
//		}
		for (PlanModel planModel : planModels) {
			...
			int result = planMapper.updateByPrimaryKeySelective(record);
			if (result != 1) {
	...
	public void deletePlanById(Integer id) {
		PlanDomain planDomain = planMapper.selectByPrimaryKey(id);
		if (planDomain == null) {
